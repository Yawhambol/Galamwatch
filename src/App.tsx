...
/**
 * GALAMSEY REPORTER (AAMUSTED) — Advanced, production-safe
 */
...
// ---- Storage ----
const LS_REPORTS = "galamsey_reports_v3";
const LS_SETTINGS = "galamsey_settings_v3";
...
export default function App() {
  ...
  const [form, setForm] = useState<any>({
    category: "", description: "", gps: null,
    blurRadius: 300, anonymous: true,
    contact: { phone: "", email: "", wantsCallback: false, preferredTime: "" },
    rewardOptIn: false, media: [] as Media[],
    stealth: false, uploadWhenSafe: true,
  ...
  useEffect(() => {
    const h = setInterval(() => {
      setReports((prev) => prev.map((r) => r.safeUpload?.required && !r.safeUpload.ready && isSafeToUpload(r, userLoc) ? { ...r, safeUpload: { ...r.safeUpload, ready: true } } : r));
    }, 30000);
  ...
  const submitReport = () => {
    ...
    if (!(form.category || form.checklistMode) || !finalDesc || !form.gps) { alert("Please select category (or use Checklist), add description, and capture GPS."); return; }
    const id = uuidv4();
    ...
    setForm({ category: "", description: "", gps: null, blurRadius: 300, anonymous: true, contact: { phone: "", email: "", wantsCallback: false, preferredTime: "" }, rewardOptIn: false, media: [], stealth: false, uploadWhenSafe: true, checklistMode: false, checklist: { types: {}, hazards: {}, time: "", risk: "" } });
    setTab("my");
  };
  ...
  const manualSync = () => {
    ...
    setTimeout(() => {
      setReports((prev) => prev.map((r) => r.status === "Submitted" ? { ...r, status: "Received", history: [...r.history, { state: "Received", at: new Date().toISOString() }] } : r));
    }, 1200);
  };
  ...
  const exportJSON = (r: Report) => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `report_${r.id}.json`; a.click(); URL.revokeObjectURL(url);
  };
...
Details: ${r.description.slice(0, 350)}`;
  ...
  const openSmsHelp = () => {
    if (!settings?.authority?.sms) alert("Set an SMS number in Settings first.");
    else alert("Open My Reports and tap 'SMS Draft' on the report you want to send.");
  };
  ...
  <span>
    ...
    {settings.safe?.maxWaitMins ?? 30} mins)
        ...
        {/* Footer actions (fixed broken <div c) */}
        <div className="mt-6 flex items-center justify-between">
          <SafetyNotes />
          <div className="flex gap-2">
            <button onClick={submitReport} className="px-4 py-2 rounded-xl bg-emerald-600 text-white">Submit Report</button>
            <button onClick={openSmsHelp} className="px-4 py-2 rounded-xl bg-gray-100">SMS Fallback</button>
          </div>
        </div>
      </Section>
                  ...
                  <div className="mt-2 text-xs">
                    {r.safeUpload?.required && (
                      <div className={needsSafe ? "text-amber-700" : "text-emerald-700"}>
                        Safe upload: <b>{needsSafe ? "Locked (move away enough or wait)" : "Ready"}</b>
                      </div>
                    )}
                  </div>
    ...
    <div className="max-w-7xl mx-auto px-3 py-4">
      <Section title="Safety & Privacy">
        ...
        <div className="mt-4 text-sm text-gray-700">
          ...
          <p className="mb-2"><b>Identity escrow:</b> If you opt for follow-up/reward, your contact stays local here; sharing to authorities needs a secure backend (future).</p>
          <p className="mb-2"><b>Offline-first:</b> Reports save locally and sync when you tap Sync or regain data.</p>
          <p className="mb-2"><b>Stealth tips:</b> Stealth hides previews and queues upload; it cannot mute the hardware shutter — please silence your device.</p>
        </div>
      </Section>
    </div>
  );
  ...
  const Settings = () => (
    <div className="max-w-3xl mx-auto px-3 py-4">
      <Section title="Authority Contacts (SMS/USSD Fallback)">
        ...
        <div className="text-xs text-gray-500 mt-2">Stored only on your device and used to open your SMS app or dialer.</div>
      </Section>
      <Section title="Upload-When-Safe Thresholds">
        ...
        <div className="text-xs text-gray-500 mt-2">A report unlocks when you move at least this distance from where it was captured, or after waiting this time.</div>
      </Section>
      <Section title="USSD Quick Dial">
        ...
        <div className="text-xs text-gray-500 mt-2">Some devices/browsers restrict USSD links. Copy & dial manually if needed.</div>
      </Section>
    </div>
  );
