 ...
 *
 * Zero extra dependencies. TSX-only.
 */
...
function randomPointInRing(lat: number, lon: number, minM: number, maxM: number) {
  const bearing = Math.random() * 2 * Math.PI; const d = minM + Math.random() * (maxM - minM);
...
type Contact = { shareContact: boolean; phone?: string; email?: string; wantsCallback?: boolean; preferredTime?: string } | null;
type Report = {
  id: string; createdAt: string; category: string; description: string;
  gps: { lat: number; lon: number; accuracy?: number };
  blurRadius: number; publicOffset: { lat: number; lon: number };
...
type Settings = {
  ...
  dpEpsilon: number;         // differential privacy epsilon (1.0 default)
  dpKMin: number;            // k-anonymity threshold
  encryptPassphrase?: string; // AES-GCM passphrase for exports
};

// ---------- Settings I/O ----------
const defaultSettings: Settings = {
  autoBlackout: true, authoritySms: "", lockMyReports: true, lockSettings: true,
  autoExpiryDays: 0, dpEpsilon: 1.0, dpKMin: 3, encryptPassphrase: "",
};
...
const loadSettings = () =>
  (typeof window === "undefined" ? defaultSettings : ({ ...defaultSettings, ...safeParse<Settings>(localStorage.getItem(LS_SETTINGS), defaultSettings) }));
const saveSettings = (s: Settings) => localStorage.setItem(LS_SETTINGS, JSON.stringify(s));
...
async function aesEncryptToBase64(plaintext: string, passphrase: string) {
  ...
  const key = await crypto.subtle.deriveKey(
    { name: "PBKDF2", salt: enc.encode("gw-salt"), iterations: 100000, hash: "SHA-256" },
...
export default function App() {
  // Basic app state
  const [tab, setTab] = useState<"report" | "my" | "map" | "help" | "settings" | "cover" | "lock">("report");
  ...
  // Uncontrolled form refs for fast typing
  ...
  useEffect(() => {
    ...
    const now = Date.now(), maxAge = settings.autoExpiryDays * 24 * 60 * 60 * 1000;
  ...
  const endMoveMonitor = () => {
    captureAnchorRef.current = null; setMoveWarn(false);
  ...
  const handleCaptureStart = (kind: CaptureKind) => {
    ...
    const msg = kind === "image" ? "Opening camera..." : kind === "video" ? "Opening video recorder..." : "Opening audio recorder...";
    setCoverReason(msg); setTab("cover");
  };
  const handleCaptureFinish = () => {
    if (!settings.autoBlackout) return;
    setTimeout(() => { setCoverReason(null); setTab("report"); }, 300);
  };
  ...
  const RedactModal: React.FC<{ idx: number }> = ({ idx }) => {
    const m = media[idx]; const [rects, setRects] = useState<{x:number;y:number;w:number;h:number}[]>([]);
    const canvasRef = useRef<HTMLCanvasElement | null>(null);
    const imgRef = useRef<HTMLImageElement | null>(null);
    const [drag, setDrag] = useState<{startX:number; startY:number; x:number; y:number; dragging:boolean}>({startX:0,startY:0,x:0,y:0,dragging:false});
    ...
    const onDown = (e: React.MouseEvent) => {
      ...
      setDrag({ startX: e.clientX - rect.left, startY: e.clientY - rect.top, x: 0, y: 0, dragging: true });
    };
    ...
    const onUp = () => {
      if (!drag.dragging) return;
      const w = drag.x - drag.startX, h = drag.y - drag.startY;
      const norm = { x: Math.min(drag.startX, drag.x), y: Math.min(drag.startY, drag.y), w: Math.abs(w), h: Math.abs(h) };
    ...
    const applyPixelate = () => {
      const c = canvasRef.current!; const ctx = c.getContext("2d")!;
      rects.forEach(r => {
        const block = 8; // pixelate size
        const temp = document.createElement("canvas"); const tctx = temp.getContext("2d")!;
        temp.width = Math.ceil(r.w / block); temp.height = Math.ceil(r.h / block);
      ...
      const newMedia = [...media]; newMedia[idx] = { ...newMedia[idx], dataUrl };
      setMedia(newMedia); setRedactIdx(null);
      ...
      <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-3">
        <div className="bg-white rounded-2xl shadow max-w-[96vw] w-full p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={()=>setRedactIdx(null)} className="px-3 py-1.5 rounded bg-gray-100">Close</button>
          </div>
          <div className="text-xs text-gray-600 mb-2">Drag to draw boxes over faces/plates. Click “Apply” to pixelate those regions.</div>
          <div className="overflow-auto">
            <canvas ref={canvasRef} onMouseDown={onDown} onMouseMove={onMove} onMouseUp={onUp} className="border rounded w-full" />
          </div>
          <div className="mt-3 flex gap-2 justify-end">
            <button onClick={()=>setRects([])} className="px-3 py-1.5 rounded bg-gray-100">Clear boxes</button>
            <button onClick={applyPixelate} className="px-3 py-1.5 rounded bg-emerald-600 text-white">Apply redaction</button>
          </div>
        </div>
      </div>
    );
  ...
  const submitReport = () => {
    ...
    if (!confirmSafe || !confirmNoConfront) {
      setBanner({ type: "error", text: "Please confirm you are at a safe distance and will not confront anyone." }); return;
    }
    const description = (descRef.current?.value || "").trim();
    if (!description) { setBanner({ type: "error", text: "Please add a short description." }); return; }

    let useGps = gps;
    const lat = Number(latRef.current?.value || ""), lon = Number(lonRef.current?.value || "");
    const latOk = isFinite(lat) && lat >= -90 && lat <= 90, lonOk = isFinite(lon) && lon >= -180 && lon <= 180;
    if (!useGps && latOk && lonOk) useGps = { lat, lon };

    if (!useGps) { setBanner({ type: "error", text: "Tap Use My Location or enter latitude and longitude." }); return; }
    ...
    if (shareContact) {
      ...
      if (wantsCallback && !phone && !email) {
        setBanner({ type: "error", text: "To request a callback, enter a phone number or an email." }); return;
      }
      contact = { shareContact: true, phone: phone || undefined, email: email || undefined, wantsCallback, preferredTime: preferredTime || undefined };
    }
    ...
    const offset = chosenBlur > 0
      ? randomPointInRing(useGps.lat, useGps.lon, Math.max(1, chosenBlur * 0.5), chosenBlur)
      : { lat: useGps.lat, lon: useGps.lon };

    const nowIso = new Date().toISOString();
    const report: Report = {
      id: uuidv4(), createdAt: nowIso, category: category || "Unspecified", description, gps: useGps,
      blurRadius: chosenBlur, publicOffset: offset, media, contact, status: "Submitted", history: [{ state: "Submitted", at: nowIso }],
    };
    setReports((prev) => [report, ...prev]);

    // reset
    setCategory(""); if (descRef.current) descRef.current.value = "";
    if (latRef.current) latRef.current.value = ""; if (lonRef.current) lonRef.current.value = "";
    if (phoneRef.current) phoneRef.current.value = ""; if (emailRef.current) emailRef.current.value = ""; if (timeRef.current) timeRef.current.value = "";
    setGps(null); setBlurRadius(sensitiveMode ? 500 : 300); setMedia([]);
    setConfirmSafe(false); setConfirmNoConfront(false); setShareContact(false); setWantsCallback(false);
    stopSafetyTimer(); endMoveMonitor();

    setBanner({ type: "success", text: "Report submitted. Track it in My Reports." });
    setTimeout(() => {
      setReports((prev) =>
        prev.map((x) => x.id === report.id ? { ...x, status: "Received", history: [...x.history, { state: "Received", at: new Date().toISOString() }] } : x)
      );
    }, 900);
    setTab("my");
  };
...
Details: ${desc}${cb}`;
  ...
  const copyToClipboard = async (txt: string) => {
    try { await navigator.clipboard.writeText(txt); setBanner({ type: "success", text: "Text copied. Paste in Messages." }); }
    catch { setBanner({ type: "info", text: "Copy failed. Long-press to select and copy from the dialog." }); window.prompt("Copy text below:", txt); }
  };
  const openSmsDraft = (r: Report) => {
    if (!settings.authoritySms) { setBanner({ type: "error", text: "Add an authority SMS number in Settings first." }); return; }
    const body = encodeURIComponent(buildSmsText(r)); const to = encodeURIComponent(settings.authoritySms);
  ...
  const lockIfNeeded = (dest: typeof tab) => {
    if ((dest === "my" && settings.lockMyReports) || (dest === "settings" && settings.lockSettings)) {
      setLockState("locked"); setLocked(true); setTab("lock");
    } else setTab(dest);
  };
  ...
  const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="bg-white rounded-2xl shadow p-4 sm:p-6 mb-5">
      <h2 className="text-lg sm:text-xl font-semibold mb-3">{title}</h2>{children}
    </section>
  );
  ...
  const NewReport = () => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-3 py-4">
        {banner && <div className={`mb-3 rounded-xl px-3 py-2 text-sm ${banner.type === "error" ? "bg-red-50 text-red-700" : banner.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-800"}`}>{banner.text}</div>}
        ...
        {showSafetyBar && (
          <div className="mb-3 rounded-xl bg-amber-50 text-amber-800 px-3 py-2 text-sm flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={()=>{ stopSafetyTimer(); endMoveMonitor(); setBanner({ type:"success", text:"Glad you’re safe."}); }} className="px-2 py-1 rounded bg-amber-100">I’m safe</button>
              <button onClick={()=>{ stopSafetyTimer(); setTimeout(startSafetyTimer, 5*60*1000); }} className="px-2 py-1 rounded bg-amber-100">Remind in 5m</button>
            </div>
          </div>
        )}
        {moveWarn && <div className="mb-3 rounded-xl bg-red-50 text-red-700 px-3 py-2 text-sm">You may be too close to the capture location. Consider moving to a safer place.</div>}

        <Section title="Reporting Form">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="cat" className="block text-sm font-medium">Category</label>
              <select id="cat" className="mt-1 w-full rounded-xl border px-3 py-2" value={category} onChange={(e) => setCategory(e.target.value)}>
                <option value="">Select (optional)</option>
                <option>River dredging</option><option>Excavator in reserve</option><option>Chemical use</option>
                <option>Night trucking</option><option>Pit hazard near school</option><option>Other</option>
              </select>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                {gps ? (<span className="text-sm text-gray-700">{gps.lat.toFixed(5)}, {gps.lon.toFixed(5)} • ±{Math.round(gps.accuracy || 0)} m</span>) : (<span className="text-sm text-gray-500">No location yet</span>)}
              </div>
              <div className="mt-2 grid grid-cols-2 gap-2">
                <input id="lat" ref={latRef} inputMode="decimal" className="rounded-xl border px-3 py-2 text-sm" placeholder="Latitude (e.g., 5.6037)" />
                <input id="lon" ref={lonRef} inputMode="decimal" className="rounded-xl border px-3 py-2 text-sm" placeholder="Longitude (e.g., -0.1870)" />
              </div>
            </div>

            <div className="sm:col-span-2">
              <label htmlFor="desc" className="block text-sm font-medium">Description</label>
              <textarea id="desc" ref={descRef} className="mt-1 w-full rounded-xl border px-3 py-2 min-h-[110px]" placeholder="What did you see? When? Any landmarks?" />
            </div>
          </div>
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <label htmlFor="safe" className="flex items-center gap-2 text-sm">
              <input id="safe" type="checkbox" checked={confirmSafe} onChange={(e) => setConfirmSafe(e.target.checked)} />
              I am at a safe distance.
            </label>
            <label htmlFor="nocon" className="flex items-center gap-2 text-sm">
              <input id="nocon" type="checkbox" checked={confirmNoConfront} onChange={(e) => setConfirmNoConfront(e.target.checked)} />
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label htmlFor="sens" className="block text-sm font-medium">Sensitive Location Mode</label>
              <label className="flex items-center gap-2 text-sm">
                <input id="sens" type="checkbox" checked={sensitiveMode} onChange={(e) => { const on = e.target.checked; setSensitiveMode(on); if (on && blurRadius < 500) setBlurRadius(500); }} />
          ...
          <div className="mt-4 grid sm:grid-cols-3 gap-3">
            <div>
              <label htmlFor="photo" className="block text-sm font-medium">Add Photo</label>
              <input id="photo" type="file" accept="image/*" capture="environment" onClick={()=>handleCaptureStart("image")} onChange={(e) => onFileChange(e, "image")} />
            </div>
            <div>
              <label htmlFor="video" className="block text-sm font-medium">Add Video</label>
              <input id="video" type="file" accept="video/*" capture="environment" onClick={()=>handleCaptureStart("video")} onChange={(e) => onFileChange(e, "video")} />
            </div>
            <div>
              <label htmlFor="audio" className="block text-sm font-medium">Add Voice Note</label>
              <input id="audio" type="file" accept="audio/*" onClick={()=>handleCaptureStart("audio")} onChange={(e) => onFileChange(e, "audio")} />
            </div>
          </div>
          {media.length > 0 && (
              ...
              {media.map((m, i) => (
                <div key={i} className="border rounded-xl p-2">
                  ...
                  <div className="mt-2 flex justify-between">
                    {m.type === "image" ? <button onClick={()=>setRedactIdx(i)} className="text-xs text-blue-600">Edit/Redact</button> : <span />}
                    <button onClick={() => removeMedia(i)} className="text-xs text-red-600">Remove</button>
                  </div>
                </div>
              ))}
            </div>
          )}
          <div className="mt-2 text-xs text-gray-600">Images remove EXIF metadata automatically. Use “Edit/Redact” to blur faces/plates.</div>

          {/* Geo-Privacy */}
          <div className="mt-4">
            <label htmlFor="blur" className="block text-sm font-medium">Geo-Privacy Blur Radius (meters)</label>
            <input id="blur" type="range" min={sensitiveMode ? 500 : 0} max={2000} step={50} value={selectedBlur} onChange={(e) => setBlurRadius(Number(e.target.value))} className="w-full" />
            <div className="flex items-center justify-between text-sm text-gray-600">
              <div className="flex gap-2">{(sensitiveMode ? [500, 700, 1000, 1500, 2000] : [0, 100, 300, 500, 1000, 2000]).map((m) => (<button key={m} onClick={() => setBlurRadius(m)} className="px-2 py-1 rounded bg-gray-100">{m}m</button>))}</div>
              <span>Selected: <b>{selectedBlur} m</b></span>
            </div>
            <p className="mt-2 text-xs text-gray-600">Public maps never show your exact point. The public pin is randomly placed inside your blur circle. Use larger blur in sensitive areas.</p>
          </div>

          {/* Contact & Callback */}
          <div className="mt-6 border-t pt-4">
            <h3 className="text-sm font-semibold mb-2">Contact & Callback (optional)</h3>
            <label className="flex items-center gap-2 text-sm mb-2"><input type="checkbox" checked={shareContact} onChange={(e) => setShareContact(e.target.checked)} /> Share contact for follow-up</label>
            {shareContact && (
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={submitReport} className={`px-4 py-2 rounded-xl text-white ${confirmSafe && confirmNoConfront ? "bg-emerald-600" : "bg-gray-400 cursor-not-allowed"}`} disabled={!confirmSafe || !confirmNoConfront}>Submit Report</button>
            </div>
          </div>
        </Section>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {reports.map((r) => (
              <div key={r.id} className="border rounded-2xl p-4">
                <div className="flex items-center justify-between mb-2"><div className="font-semibold text-sm">{r.category}</div><div className="text-xs text-gray-500">{new Date(r.createdAt).toLocaleString()}</div></div>
                ...
                {r.contact?.shareContact && (
                  <div className="mb-2 text-xs">
                    <span className="inline-block px-2 py-0.5 rounded bg-blue-50 text-blue-700 mr-2">Contact Shared</span>
                    {r.contact?.wantsCallback && <span className="inline-block px-2 py-0.5 rounded bg-amber-50 text-amber-700">Callback requested</span>}
                    <div className="mt-1 text-gray-700">
                      {r.contact?.phone && <div>Phone: {r.contact.phone}</div>}
                      {r.contact?.email && <div>Email: {r.contact.email}</div>}
                      {r.contact?.preferredTime && <div>Preferred time: {r.contact.preferredTime}</div>}
                    </div>
                  </div>
                )}
                <div className="mb-2"><Timeline status={r.status} /></div>
                {r.media?.length > 0 && (
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button onClick={() => exportJSON(r, `report_${r.id}.json`)} className="px-3 py-1.5 rounded-xl bg-gray-900 text-white text-xs">Export JSON</button>
                  <button onClick={() => exportJSON(redactedClone(r), `report_${r.id}_redacted.json`)} className="px-3 py-1.5 rounded-xl bg-gray-100 text-gray-800 text-xs">Export Redacted</button>
                  <button
                    onClick={async () => {
                      if (!settings.encryptPassphrase) { setBanner({ type: "error", text: "Set an encryption passphrase in Settings first." }); return; }
                      const ct = await aesEncryptToBase64(JSON.stringify(r), settings.encryptPassphrase);
                      exportJSON({ encrypted: true, alg: "AES-GCM", payload_b64: ct }, `report_${r.id}_encrypted.json`);
                    }}
                  ...
                  <button onClick={() => openSmsDraft(r)} className="px-3 py-1.5 rounded-xl bg-blue-600 text-white text-xs">SMS Draft</button>
                  <button onClick={() => copyToClipboard(buildSmsText(r))} className="px-3 py-1.5 rounded-xl bg-blue-100 text-blue-700 text-xs">Copy SMS Text</button>
                  {r.status !== "Resolved" && (
                    <button
                      onClick={() =>
                        setReports((prev) =>
                          prev.map((x) => x.id === r.id
                            ? { ...x, status: x.status === "In Progress" ? "Resolved" : "In Progress", history: [...x.history, { state: x.status === "In Progress" ? "Resolved" : "In Progress", at: new Date().toISOString() }] }
                            : x
                          )
                        )
                      }
  ...
  function dpNoisy(count: number, epsilon: number) {
    // Laplace noise with scale = 1/epsilon
      ...
      <div className="max-w-7xl mx-auto px-3 py-4">
        <Section title="Map & Distance">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              {!privateView && <label className="text-sm flex items-center gap-2 bg-gray-100 px-2 py-1.5 rounded-xl"><input type="checkbox" checked={heat} onChange={(e)=>setHeat(e.target.checked)} /> Heatmap</label>}
    ...
    return (
      <div className="max-w-3xl mx-auto px-3 py-4">
        <Section title="Security">
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            <label className="flex items-center gap-2"><input type="checkbox" checked={settings.lockMyReports} onChange={(e)=>setSettings({...settings, lockMyReports: e.target.checked})} /> Lock “My Reports”</label>
            <label className="flex items-center gap-2"><input type="checkbox" checked={settings.lockSettings} onChange={(e)=>setSettings({...settings, lockSettings: e.target.checked})} /> Lock “Settings”</label>
            <div className="sm:col-span-2 grid sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium">Set/Change App PIN</label>
                <input className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="4–6 digits" value={pin} onChange={(e)=>setPin(e.target.value)} inputMode="numeric" />
                <button className="mt-2 px-3 py-1.5 rounded bg-gray-900 text-white text-xs"
                  onClick={async ()=>{ if (!pin) return; const h = await sha256Hex(pin); setSettings({...settings, appPinHash: h}); setPin(""); setBanner({ type:"success", text:"PIN saved."}); }}>
                  Save PIN
                </button>
              </div>
              <div>
        ...
        <Section title="Authority & Exports">
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <input className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="e.g., 190 or +233XXXXXXXXX" value={settings.authoritySms} onChange={(e) => setSettings({ ...settings, authoritySms: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium">Encryption passphrase (AES-GCM)</label>
              <input className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="Set to enable encrypted export" value={settings.encryptPassphrase || ""} onChange={(e)=>setSettings({...settings, encryptPassphrase: e.target.value})} />
            </div>
          </div>
          <p className="mt-2 text-xs text-gray-600">“Export Encrypted” uses your passphrase to encrypt JSON locally (AES-GCM). Share passphrase securely with the authority.</p>
        </Section>

        <Section title="Privacy & Cleanup">
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            <label className="flex items-center gap-2"><input type="checkbox" checked={settings.autoBlackout} onChange={(e)=>setSettings({...settings, autoBlackout: e.target.checked})} /> Auto Blackout during capture (switch to Notes cover)</label>
            <div>
              <label className="block text-sm font-medium">Auto-expire after (days)</label>
              <input type="number" min={0} className="mt-1 w-full rounded-xl border px-3 py-2" value={settings.autoExpiryDays} onChange={(e)=>setSettings({...settings, autoExpiryDays: Math.max(0, Number(e.target.value)||0)})} />
              <div className="text-xs text-gray-500 mt-1">0 = off. Old reports will be removed at next app start.</div>
            </div>
            <div>
              <label className="block text-sm font-medium">DP epsilon (heatmap noise)</label>
              <input type="number" step="0.1" min={0.1} className="mt-1 w-full rounded-xl border px-3 py-2" value={settings.dpEpsilon} onChange={(e)=>setSettings({...settings, dpEpsilon: Math.max(0.1, Number(e.target.value)||1)})} />
            </div>
            <div>
              <label className="block text-sm font-medium">DP k-anonymity (min cells)</label>
              <input type="number" min={1} className="mt-1 w-full rounded-xl border px-3 py-2" value={settings.dpKMin} onChange={(e)=>setSettings({...settings, dpKMin: Math.max(1, Number(e.target.value)||3)})} />
            </div>
          </div>
        </Section>

        <Section title="Learn: How to use the app">
          <button onClick={()=>setShowLearn(v=>!v)} className="px-3 py-1.5 rounded bg-gray-900 text-white text-sm">{showLearn ? "Hide guide" : "Open guide"}</button>
          {showLearn && (
  ...
  const NotesCover = () => (
    <div className="max-w-3xl mx-auto px-3 py-4">
      <Section title="My Notes">
        ...
        <textarea className="w-full rounded-xl border px-3 py-2 min-h-[260px]" placeholder="Write notes..." value={notes} onChange={(e) => setNotes(e.target.value)} />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header />
      {banner && <div className={`max-w-7xl mx-auto px-3 mt-2`}><div className={`rounded-xl px-3 py-2 text-sm ${banner.type === "error" ? "bg-red-50 text-red-700" : banner.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-800"}`}>{banner.text}</div></div>}
      {tab === "lock" && <LockScreen />}
      {tab !== "lock" && (
        <>
      ...
      <footer className="max-w-7xl mx-auto px-3 py-6 text-xs text-gray-500">Demo only • All data stored locally • AAMUSTED project</footer>
    </div>
  );
}
